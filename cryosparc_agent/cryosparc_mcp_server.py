...
from proshade_adapter import analyze_volume_symmetry as analyze_volume_symmetry_impl
...
@mcp.tool()
def select_available_compute_lane(
    preferred_lanes: list[str] | None = None,
    requires_gpu: bool = True,
) -> dict:
    """Select a scheduler lane with observed capacity; does not submit a Job."""
    return select_available_lane(preferred_lanes=preferred_lanes, requires_gpu=requires_gpu)
...
def analyze_volume_symmetry(
    ...
    """Run optional ProSHADE symmetry detection on a completed 3D volume."""
    return analyze_volume_symmetry_impl(
        project_uid=project_uid,
        job_uid=job_uid,
