...
def quality_rollback_enabled() -> bool:
    """Enable quality-driven upstream recovery only for opted-in runs."""
    return os.getenv("CRYOAGENT_ENABLE_QUALITY_ROLLBACK", "0").lower() in {
        "1", "true", "yes", "on"
    }
...
def build_decision_guidance(
    ...
    rollback_candidates = [
        ...
        if item.get("quality_rollback") and item.get("available")
    ]
    if rollback_candidates:
        quality_rollback = {
            "priority": "high",
            "candidate_action_ids": [
                item.get("action_id") for item in rollback_candidates
            ],
            "candidate_branches": [
                {
                    "action_id": item.get("action_id"),
                    "class2d_job_uid": (item.get("quality_rollback_branch") or {}).get(
                        "class2d_job_uid"
                    ),
                    "box_size_pix": (item.get("quality_rollback_branch") or {}).get(
                        "box_size_pix"
                    ),
            ...
            "model_instruction": (
                "The current refinement quality is below the explicit target. Prefer the "
                "quality rollback candidates to revisit upstream particle selection before "
                "starting another refinement with the same particles. Compare the visual "
                "evidence for every listed Class 2D box-size branch when available; otherwise "
                "use CryoSift scores and class statistics as structured evidence. Then choose "
                "exactly one candidate action_id; do not automatically choose the latest branch."
            ),
        }
    return {
