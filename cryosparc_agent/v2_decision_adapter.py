...
def adapt_v2_decision_to_internal(
    ...
    if decision_type in {"stop", "rollback", "request_input"}:
...
def normalize_requested_actions(v2_decision: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    # Some API models use ``actions`` for an exploratory box-size sweep even
    # when they label the overall decision as ``forward``. Treat it as the
    # explicit action list; validation and candidate matching still apply to
    # every member before anything is submitted.
    actions = v2_decision.get("actions")
