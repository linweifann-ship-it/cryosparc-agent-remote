...
async def main_async() -> None:
    ...
    async with AsyncExitStack() as stack:
        ...
        for round_index in range(1, args.max_rounds + 1):
            ...
            append_feedback_to_recent_job_history(model_input, feedback_to_model)
            ...
            if decision_type in {"stop", "complete"}:
                ...
                round_log["next_round_reason"] = f"Model returned {decision_type}."
                break
...
def build_autonomous_prompt(
    ...
    payload = {
        "round": round_index,
        "instruction": (
            "Choose exactly one next action or stop from the current "
            "CryoSPARC state. Do not repeat completed Import Micrographs. "
            "MCP resolves CryoSPARC input connections; do not output connections, "
            "reasoning, confidence, evidence, or markdown."
        ),
        "model_input": model_input,
...
def append_feedback_to_recent_job_history(
    model_input: dict[str, Any],
    feedback: dict[str, Any] | None,
) -> None:
    """Fold runner feedback into schema 2.1 current_state.recent_job_history."""
    failure_context = build_failure_context(feedback)
    if not failure_context:
        return
    current_state = model_input.get("current_state")
    if not isinstance(current_state, dict):
        return
    history = current_state.setdefault("recent_job_history", [])
    if not isinstance(history, list):
        current_state["recent_job_history"] = []
        history = current_state["recent_job_history"]
    failed_action = failure_context.get("failed_action") or {}
    attempted_job = failure_context.get("attempted_job") or {}
    execution_error = failure_context.get("execution_error") or {}
    job_type = (
        attempted_job.get("job_type")
        or failed_action.get("job_type")
        or failed_action.get("action")
        or failure_context.get("failure_stage")
    )
    previous_attempts = [
        item
        for item in history
        if isinstance(item, dict) and item.get("job_type") == job_type
    ]
    history.append(
        {
            "job_uid": None,
            ...
            "status": "failure",
            "key_parameters": failed_action.get("parameters") or {},
            "output_summary": {
                "output_group_names": [],
                "output_field_names": [],
                "num_items_by_group": {},
            ...
            "error_summary": {
                "error_code": failure_context.get("failure_stage") or "MCP_FAILURE",
                "message": execution_error.get("raw_error_text")
                or execution_error.get("message")
                or "MCP failure feedback.",
            },
        }
    )
    current_state["recent_job_history"] = history[-6:]
