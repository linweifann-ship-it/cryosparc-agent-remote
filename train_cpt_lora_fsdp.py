#!/usr/bin/env python3
"""FSDP + LoRA CPT training on paper chunks."""
...
class CausalTextDataset(Dataset):
    def __init__(self, records: list[dict[str, Any]], tokenizer, max_length: int, text_field: str):
        ...
        if not self.examples:
            raise ValueError("No usable CPT samples were produced after tokenization.")
        print(f"Prepared {len(self.examples)} CPT samples; skipped {skipped}.")
...
def main() -> None:
    ...
    train_dataset = CausalTextDataset(train_records, tokenizer, args.max_length, args.text_field)
    eval_dataset = CausalTextDataset(eval_records, tokenizer, args.max_length, args.text_field) if eval_records else None
    ...
    trainer = Trainer(
        ...
        data_collator=CPTDataCollator(tokenizer),
    )
